...
def command_search_entry():
    listBox.delete(0, END)
    for row in backend.search_entry(
        text_Title.get(), text_Author.get(), text_Year.get(), text_ISBN.get()
    ):
        listBox.insert(END, row)
...
but_searchentry = Button(
    window, text="Search Entry", width=12, command=command_search_entry
)
