...
for coordinate,el in zip(coordinates,elevation):
    ...
    fgv.add_child(folium.CircleMarker(location=coordinate,radius=6,popup=folium.Popup(iframe),fill_color=colour(el,mean),color='grey',fill_opacity=0.9))
...
map.add_child(fgv)
map.add_child(fgp)
map.add_child(folium.LayerControl()) #This needs to come after all layers are created
